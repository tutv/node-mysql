...
app.get('/api/2', function (req, res) {
    ...
    knex.select('posts.*', 'authors.name', 'authors.username').from('posts').innerJoin('authors', 'posts.author_id', '=', 'authors.id').innerJoin('post_categories', 'post_categories.post_id', '=', 'post.id').innerJoin('categories', 'categories.id', '=', 'post_categories.category_id').limit(10).then(function (posts) {
        var doneTime = datek.getNowTimestamp();
        var sumTime;
        sumTime = doneTime - startTime;
